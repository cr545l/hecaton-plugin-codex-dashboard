...
// 넘칠 만한 줄(특히 최소화 바)은 반드시 여기를 통과시킬 것 — 이유는 renderMinimized 주석.
  ...
  function renderMinimized(state) {
    ...
    if (d) {
      if (d.primary) {
        ...
        addTooltipArea(segStart, windowLabel + ' limit', d.primary.resetsAt);
      }
      if (d.secondary) {
        ...
        line += colors.label + (reset || windowLabel) + ': ' + ansi.reset;
        line += formatPercent(pct) + ' ' + progressBar(pct, 10);
        addTooltipArea(segStart, windowLabel + ' limit', d.secondary.resetsAt);
      }
      if (d.timestamp) {
    ...
    // ⚠️ 조건이 `!d` 면 안 된다 — 스캔은 끝났는데 rate limit 항목이 하나도 없으면
    ...
    if (plainLength() === 0) {
      if (state.error) {
        ...
        line += colors.dim + 'Scanning ' + p.current + '/' + p.total + ansi.reset;
    ...
    // ⚠️ 자르기가 빠지면 안 된다 — 최소화 바는 호스트 버퍼가 1행이라, 폭을 한 칸이라도
